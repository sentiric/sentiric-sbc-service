...
impl AppConfig {
    pub fn load_from_env() -> Result<Self> {
        let grpc_port =
            env::var("SIP_SBC_SERVICE_GRPC_PORT").unwrap_or_else(|_| "13091".to_string());
        let http_port =
            env::var("SIP_SBC_SERVICE_HTTP_PORT").unwrap_or_else(|_| "13090".to_string());

        let sip_port_str =
            env::var("SIP_SBC_SERVICE_SIP_PORT").unwrap_or_else(|_| "13094".to_string());
        let sip_port = sip_port_str.parse::<u16>().context("Geçersiz SIP portu")?;
        let advertised_port = env::var("SIP_SBC_ADVERTISED_PORT")
        ...
        let proxy_target = env::var("SIP_PROXY_SERVICE_SIP_TARGET")
            .context("ZORUNLU: SIP_PROXY_SERVICE_SIP_TARGET eksik")?;
        let public_ip =
            env::var("SIP_SBC_SERVICE_PUBLIC_IP").unwrap_or_else(|_| "127.0.0.1".to_string());
        let internal_ip =
            env::var("SIP_SBC_SERVICE_INTERNAL_IP").unwrap_or_else(|_| "127.0.0.1".to_string());
        let b2bua_port = env::var("SIP_B2BUA_SERVICE_SIP_PORT")
            .unwrap_or_else(|_| "13084".to_string())
            .parse::<u16>()?;
        let rtp_start = env::var("SIP_SBC_RTP_START_PORT")
            .unwrap_or_else(|_| "30000".to_string())
            .parse()?;
        let rtp_end = env::var("SIP_SBC_RTP_END_PORT")
            .unwrap_or_else(|_| "30100".to_string())
            .parse()?;

        //[ARCH-COMPLIANCE] tenant_isolation kuralı zorlaması. Boş bırakılması YASAKTIR.
        let tenant_id =
            env::var("TENANT_ID").context("ZORUNLU: TENANT_ID çevre değişkeni eksik")?;

...
use sentiric_sip_core::{Header, HeaderName, Method, SipPacket, SipRouter};
...
impl SbcEngine {
    ...
    fn apply_smart_topology_hiding(&self, packet: &mut SipPacket, src_addr: SocketAddr) {
        ...
        let old_contact_val = packet
            .get_header_value(HeaderName::Contact)
            .cloned()
            .unwrap_or_default();
        let user_part = sip_utils::extract_username_from_uri(&old_contact_val);
        let final_user = if user_part.is_empty() {
            "sbc".to_string()
        } else {
            user_part
        };
        ...
        packet
            .headers
            .insert(0, Header::new(HeaderName::RecordRoute, rr_val));

        if !is_register {
            let is_internal_contact = old_contact_val.contains("10.")
                || old_contact_val.contains("192.168.")
                || old_contact_val.contains("172.")
                || old_contact_val.contains("100.");
            let is_external_src = !is_internal_ip(src_addr.ip());

            if is_external_src {
                packet.headers.retain(|h| h.name != HeaderName::Contact);
                let contact_val =
                    format!("<sip:{}@{}:{}>", final_user, src_addr.ip(), src_addr.port());
                packet
                    .headers
                    .push(Header::new(HeaderName::Contact, contact_val));
        ...
        packet
            .headers
            .retain(|h| h.name != HeaderName::Server && h.name != HeaderName::UserAgent);
        packet
            .headers
            .push(Header::new(HeaderName::Server, "Sentiric-SBC".to_string()));
    ...
    fn fix_request_uri_for_internal(&self, packet: &mut SipPacket) {
        ...
        if packet.uri.contains(&public_port_str) || !packet.uri.contains(':') {
            if packet.uri.contains(':') {
                packet.uri = packet
                    .uri
                    .replace(&public_port_str, &format!(":{}", internal_port));
            } else {
